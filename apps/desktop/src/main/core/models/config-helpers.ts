...
/** contextWindow 缺省估算值：getContextPressure 与 UI「估算」标记共用。 */
...
/** 有效 contextWindow：未填写时回落 128k 估算。 */
export function effectiveContextWindow(contextWindow?: number) {
  return toOptionalContextWindow(contextWindow) ?? ESTIMATED_CONTEXT_WINDOW;
}
