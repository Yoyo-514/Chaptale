...
export type ModelConfigRepositoryOptions = {
  modelsPath: string;
...
export class ModelConfigRepository {
  ...
  constructor(private readonly options: ModelConfigRepositoryOptions) {}
  ...
  update(mutator: (config: ModelsConfig) => void | Promise<void>): Promise<void> {
  ...
  async getDefaultModel(): Promise<ModelRef | undefined> {
    const config = await this.read().catch(() => ({ providers: {} }) as ModelsConfig);

    return config.defaultModel;
  }

  async setDefaultModel(ref: ModelRef | undefined): Promise<void> {
  ...
  private async readImmediately(): Promise<ModelsConfig> {
    try {
  ...
  private async writeImmediately(config: ModelsConfig): Promise<void> {
