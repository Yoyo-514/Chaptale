...
import { PiCustomModelConfigService } from '../pi-custom-model-config.service';
...
function cloneConfig(config: PiModelsConfig): PiModelsConfig {
  return JSON.parse(JSON.stringify(config)) as PiModelsConfig;
}

function createRepository(initial: PiModelsConfig = { providers: {} }) {
  let config = cloneConfig(initial);
  const repository = {
    read: vi.fn(async () => config),
    write: vi.fn(async (next: PiModelsConfig) => {
      config = cloneConfig(next);
    }),
    findCustomModel: vi.fn((nextConfig: PiModelsConfig, provider: string, modelId: string) => {
  ...
  } satisfies Pick<PiModelConfigRepository, 'read' | 'write' | 'findCustomModel'>;

  return { repository, getConfig: () => config };
}

describe('PiCustomModelConfigService', () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('resolves fetch source from an existing provider or from a draft payload', async () => {
    ...
    const service = new PiCustomModelConfigService(repository as unknown as PiModelConfigRepository);
