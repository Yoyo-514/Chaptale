...
import { getFileKind, getImageMimeType } from './context-files/file-kind';
...
export type PiImageBlock = ImageContent & {
  blockIndex: number;
};
...
export function decodeImageBase64(image: Pick<ImageContent, 'data' | 'mimeType'>) {
...
export function getPiUserImageBlocks(message: unknown): PiImageBlock[] {
  ...
  return record.content.flatMap((item, blockIndex) => {
    ...
    return [{ type: 'image', data: block.data, mimeType: block.mimeType, blockIndex } satisfies PiImageBlock];
  });
}
...
export class ImageAttachmentService {
  ...
  readOriginal(message: unknown, blockIndex: number) {
    const image = getPiUserImageBlocks(message).find(item => item.blockIndex === blockIndex);
