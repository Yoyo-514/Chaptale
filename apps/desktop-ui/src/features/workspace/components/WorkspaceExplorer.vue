...
function prepareContext(event: MouseEvent) {
  const element = event.target instanceof Element ? event.target.closest<HTMLElement>('[data-tree-path]') : null;
  contextPath.value = element?.dataset.treePath ?? '';
  tree.selectedPath = contextPath.value;
  const row = rows.value.find(item => item.relativePath === contextPath.value);
  const file = row?.kind === 'file';
  const writable =
    Boolean(row) &&
    !contextPath.value.split('/').some(part => part.startsWith('.')) &&
    contextPath.value !== 'chaptale.json';
  contextItems.value = [
    ...(file
      ? [
          { id: 'open', label: '打开', icon: 'i-mingcute-file-line' },
          { id: 'agent', label: '与 Agent 讨论此文件', icon: 'i-mingcute-chat-3-line' },
          { id: 'reference', label: '加入写作参考', icon: 'i-mingcute-bookmark-add-line' }
        ]
      : [
          { id: 'new-file', label: '新建文件…', icon: 'i-mingcute-file-new-line', disabled: !workspace.rootPath },
          {
            id: 'new-directory',
            label: '新建文件夹…',
          ...
          { id: 'new-template', label: '从模板新建…', disabled: !workspace.rootPath },
          { id: 'collapse', label: '折叠全部文件夹' }
        ]),
    ...(row
      ? [
          { id: 'rename', label: '重命名…', shortcut: 'F2', separatorBefore: true, disabled: !writable },
          { id: 'move', label: '移动到…', disabled: !writable },
          ...(file ? [{ id: 'duplicate', label: '复制为新文件…', disabled: !writable }] : []),
          { id: 'copy-path', label: '复制相对路径', separatorBefore: true },
          { id: 'copy-absolute', label: '复制绝对路径' }
        ]
      : []),
    { id: 'reveal', label: '在系统文件管理器中显示', separatorBefore: true, disabled: !workspace.rootPath },
    { id: 'refresh', label: '刷新', icon: 'i-mingcute-refresh-3-line', disabled: !workspace.rootPath },
    ...(row
      ? [
          {
            id: 'trash',
...
function selectContext(id: string) {
  ...
  if (['rename', 'move', 'duplicate', 'trash'].includes(id))
    void actions.prepare(id as 'rename' | 'move' | 'duplicate' | 'trash', path);
  else if (id === 'open') emit('openFile', path);
  else if (id === 'agent') actions.askAgent(path);
  else if (id === 'reference') {
    library.add(path);
    navigation.showAuxiliary('references');
  } else if (id === 'new-file' || id === 'new-directory')
    void tree.startCreation(id === 'new-file' ? 'file' : 'directory');
  else if (id === 'new-template') void templates.openCreate();
  else if (id === 'copy-path' || id === 'copy-absolute') void actions.copyPath(path, id === 'copy-absolute');
  else if (id === 'reveal') void actions.reveal(path);
  else if (id === 'refresh') void tree.reload();
  else if (id === 'collapse') tree.collapseAll();
}
...
function handleKeydown(index: number, event: KeyboardEvent) {
  ...
  if (event.key === 'F2' || event.key === 'Delete') {
    ...
    void actions.prepare(event.key === 'F2' ? 'rename' : 'trash', row.relativePath);
