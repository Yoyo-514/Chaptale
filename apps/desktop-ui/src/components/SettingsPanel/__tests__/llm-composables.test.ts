...
function createSettingsStore() {
  return {
    ...
    removeProviderAuth: vi.fn().mockResolvedValue(true),
...
describe('LLM settings composables', () => {
  ...
  it('submits provider API keys for builtin and custom groups and clears successful input', async () => {
    ...
    expect(auth.getKeyPlaceholder({ authConfigured: true } as any)).toBe('••••••••••••');
    ...
    expect(auth.isKeySaving('custom')).toBe(false);

    await auth.removeProviderAuth('custom');
